...
const requestGetLeagueStandingsDirectly = (idSeason:string) => { 
    return axios.get(`https://app.sportdataapi.com/api/v1/soccer/standings?apikey=${process.env.REACT_APP_SPORT_DATA_API_API_KEY}&season_id=${idSeason}`)
};

const requestSetLeagueStandings = (idLeague: string, leagueStandings: any) => {
    return firebaseFirestore.collection("Football.LeagueStandings_").doc(idLeague).set(leagueStandings) 
};


// directly access to sportdataAPI -> update firebase (get document on return)
function* updateLeagueStandings(action: actions.data.football.type__UPDATE_LEAGUE_STANDINGS) {

    const {idLeague, triggeringGet} = action.payload;

    try {
            
        const idUser: string =  yield select( (state:StateRoot) => state.auth.user?.id); 

        const {data: {data: listSeason}} =  yield call( requestGetListSeasonDirectly, idLeague ); 
        const seasonCurrent = listSeason.find((seasonEach:any) => seasonEach.is_current === 1);

        const {season_id : idSeason, name: nameSeason, country_id: idCountry, start_date: dateStart, end_date: dateEnd } = seasonCurrent;
        
        const {data: {data: dataLeagueStandings}} =  yield call( requestGetLeagueStandingsDirectly, idSeason );
        const {has_groups, standings} = dataLeagueStandings;
        ...
        */
        ...
        yield call( requestSetLeagueStandings, idLeague, draftLeagueStandings );

        if (triggeringGet){
            yield put (actions.data.football.return__GET_LEAGUE_STANDINGS({
                idLeague: idLeague
            }));
        }
...
export default updateLeagueStandings;
